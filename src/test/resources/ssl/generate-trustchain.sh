...
#  REQUIREMENTS:  ---
#          BUGS:  ---
#         NOTES:  Modified and run to be compatible with recent JDKs by sgala@redhat.com
...
function generate_intermediate_signed_cert_using_extension() {
    ...
    #private key
    ossl genpkey -algorithm rsa \
        -pass pass:${PASSPHRASE} \
        -out ${INTERMEDIATE_DIR}/private/${NAME}.key.pem -pkeyopt rsa_keygen_bits:2048 \
...
function create_jks_keystore_for_cert() {
    ...
    ossl pkcs12 -export \
        ...
        -passin pass:${PASSPHRASE} \
        -passout pass:${PASSPHRASE} -keypbe aes-256-cbc -certpbe aes-256-cbc \
        -inkey "${INTERMEDIATE_DIR}/private/${CERT_NAME}.key.pem" \
        -certfile "${INTERMEDIATE_DIR}/certs/${TRUSTCHAIN_NAME}.cert.pem" \
        -name "${KEYSTORE_ALIAS}" -nodes -out "${INTERMEDIATE_DIR}/keystores/$KEYSTORE_NAME.keystore.p12" \
        && printlog "Created PKCS#12 keystore with ${CERT_NAME}.cert.pem"
    # As of 2022-01-29 it will fail when run on a fips-enabled Red Hat java VM
    # with error message keytool error: java.io.IOException: parseAlgParameters failed: PBES2 AlgorithmParameters not available
    ktool -J-Dcom.redhat.fips=false -importkeystore -srckeystore "${INTERMEDIATE_DIR}/keystores/$KEYSTORE_NAME.keystore.p12" -srcstoretype pkcs12 -srcstorepass ${PASSPHRASE} \
...
ossl genpkey -algorithm rsa -out private/ca.key.pem -pass pass:${PASSPHRASE} -pkeyopt rsa_keygen_bits:4096 && printlog "Generated root key!"
...
ossl genpkey -algorithm rsa \
       ...
       -pass pass:${PASSPHRASE} \
       -pkeyopt rsa_keygen_bits:4096 && printlog "Generated intermediate key!"
...
ktool -import -file "certs/ca.cert.pem" \
        ...
        -noprompt \
        -storetype jks -storepass "${PASSPHRASE}" && \
...
ktool -import -alias ca -file ${CA_DIR}/certs/ca.cert.pem -keystore ${INTERMEDIATE_DIR}/keystores/ca-chain.keystore.jks -storepass ${PASSPHRASE} -storetype jks -noprompt
ktool -import -alias intermediate -file ${INTERMEDIATE_DIR}/certs/intermediate.cert.pem -keystore ${INTERMEDIATE_DIR}/keystores/ca-chain.keystore.jks -storepass ${PASSPHRASE} -storetype jks -noprompt
